...
let Slider = class Slider extends Component {
    ...
    scaleAlignment() {
        ...
        if (orien === 'v') {
            let tickElements = this.ul.querySelectorAll('.' + classNames.large + ' .' + classNames.tickValue);
            let ticksSpan = [].slice.call(tickElements, 0);
            for (let i = 0; i < ticksSpan.length; i++) {
                if (!isNullOrUndefined(this.ticks) && !isNullOrUndefined(this.ticks.placement)) {
                    if (this.ticks.placement === 'after') {
                        ...
                        let width = this.sliderTrack.clientWidth + largeTick;
...
let Uploader = class Uploader extends Component {
    ...
    getKeyValue(val) {
        let keyValue;
        for (let key of Object.keys(this.localeText)) {
    ...
    removeUploadedFile(file) {
        ...
        ajax.addEventListener('load', (e) => { this.removeCompleted(e, selectedFiles); }, false);
        /* istanbul ignore next */
        ajax.addEventListener('error', (e) => { this.removeFailed(e, selectedFiles); }, false);
        ajax.open('POST', this.asyncSettings.removeUrl);
        ajax.send(formData);
    }
    removeCompleted(e, files) {
        let args = {
            e, operation: 'remove', file: this.updateStatus(files, this.localizedTexts('removedSuccessMessage'))
        };
    ...
    removeFailed(e, files) {
        let args = {
            e, operation: 'remove', file: this.updateStatus(files, this.localizedTexts('removedFailedMessage'))
        };
    ...
    uploadInProgress(e, files, customUI) {
        ...
        let args = { e, operation: 'upload', file: this.updateStatus(files, this.localizedTexts('inProgress')) };
    ...
    uploadComplete(e, files, customUI) {
        ...
        if (status.readyState === 4 && status.status === 200) {
            ...
            let args = { e, operation: 'upload', file: this.updateStatus(files, this.localizedTexts('uploadSuccessMessage')) };
            this.trigger('success', args);
    ...
    uploadFailed(e, files) {
        let li = this.getLiElement(files);
        let args = { e, operation: 'upload', file: this.updateStatus(files, this.localizedTexts('uploadFailedMessage')) };
        if (!isNullOrUndefined(li)) {
    ...
    upload(files, custom) {
        ...
        for (let i = 0; i < selectedFiles.length; i++) {
            let ajax = new XMLHttpRequest();
            let formData = new FormData();
            if (selectedFiles[i].status === this.localizedTexts('readyToUploadMessage')) {
                ...
                this.trigger('uploading', { file: this.updateStatus(selectedFiles[i]) });
                ajax.send(formData);
            }
        }
    }

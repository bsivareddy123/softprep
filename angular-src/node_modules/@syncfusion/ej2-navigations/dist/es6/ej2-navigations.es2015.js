...
let HScroll = class HScroll extends Component {
    ...
    destroy() {
        ...
        let nav = selectAll('#' + ele.id + '_nav.e-' + ele.id + '_nav');
    ...
    createNavIcon(element) {
        ...
        let nav = createElement('div', { id: id, className: clsRight });
        ...
        let navEle = createElement('div', { id: id, className: clsLeft + ' ' + CLS_DISABLE });
...
let Tab = class Tab extends Component {
    ...
    parseObject(items, index) {
        ...
        items.forEach((item, i) => {
            ...
            let disabled = (item.disabled) ? ' ' + CLS_DISABLE$3 : '';
    ...
    removeActiveClass(id) {
        ...
        if (!isNullOrUndefined(hdrActEle)) {
            ...
            let trg = this.findEle(select('.e-content', this.element).children, CLS_CONTENT$1 + '_' + no);
        }
    }
    ...
    setActive(value) {
        ...
        if (this.isTemplate) {
            ...
            let item = select('.' + CLS_CONTENT$1 + ' > #' + CLS_CONTENT$1 + '_' + this.extIndex(id), this.element);
            if (isNullOrUndefined(item)) {
                ...
                let eleTrg = select('.' + CLS_CONTENT$1 + ' > #' + CLS_CONTENT$1 + '_' + this.extIndex(id), this.element);
    ...
    removeTab(index) {
        ...
        let cntTrg = this.findEle(select('.e-content', this.element).children, CLS_CONTENT$1 + '_' + no);
...
let TreeView = class TreeView extends Component {
    ...
    beginEdit(node) {
        ...
        if (!isNullOrUndefined(ele) && this.allowEditing) {
...
let Sidebar = class Sidebar extends Component {
    ...
    documentclickHandler(e) {
        if (e.target.closest('.' + CONTROL + '' + '.' + ROOT$1)) {
            return;
        }
        this.hide();
    }

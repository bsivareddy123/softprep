...
let Dialog = class Dialog extends Component {
    ...
    getZindexPartial() {
        // upto body traversal
        let dialogParent = this.element.parentElement;
        let parentZindex = [];
        while (dialogParent) {
            if (dialogParent.tagName !== 'BODY') {
                let index = document.defaultView.getComputedStyle(dialogParent, null).getPropertyValue('z-index');
                let position = document.defaultView.getComputedStyle(dialogParent, null).getPropertyValue('position');
...
let Tooltip = class Tooltip extends Component {
    ...
    formatPosition() {
        if (this.position.indexOf('top') === 0 || this.position.indexOf('bottom') === 0) {
            [this.tooltipPositionY, this.tooltipPositionX] = this.position.split(' ');
        }
        else {
            [this.tooltipPositionX, this.tooltipPositionY] = this.position.split(' ');
        }
    }
    renderArrow() {
...
function fb_calculate_attributes(radius, innerConainer) {
    ...
    let startArc = 315;
    let endArc = 45;
    let svg = innerConainer.querySelector('.e-spin-fabric');
...
export { Popup, getScrollableParent, calculateRelativeBasedPosition, calculatePosition, fit, isCollide, flip, ButtonProps, Dialog, Animation$1 as Animation, Tooltip, createSpinner, showSpinner, hideSpinner, setSpinner };
